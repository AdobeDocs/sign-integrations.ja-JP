...
* Adobe Signで管理アカウントをアクティベートします（新規顧客のみ）。
...
設定するには [!DNL Veeva Vault] Adobe Signと統合するには、以下の手順を実装する必要があります。
...
1. 管理者は、既存の共有フィールドを追加する必要があります *Vault オーバーレイを無効にする (disable_vault_overlays__v)* を選択し、 **[!UICONTROL アクティブ]** Adobe署名の対象となるすべての文書タイプ 必要に応じて、Adobe Sign Admin グループのメンバーのみが値を更新できる特定のセキュリティをフィールドに設定できます。

   ![Adobe Sign ユーザーアクションを許可の画像](images/allow-adobe-sign-user-actions.png)

### 手順 8. 文書のレンディションを宣言 {#declare-renditions}

新しいレンディションのタイプは *Adobe Signレンディション (adobe_sign_rendition__c)* は、Vault との統合によって、署名済みPDF文書をAdobe Signにアップロードします。 Adobe署名の対象となる各文書タイプに対して、Adobe Signレンディションを宣言する必要があります。
...
1. ライフサイクルロールの追加。 Adobe Sign管理者アプリケーションロールは、以下に示すように、アプリケーション署名の対象となる文書で使用されるすべてのAdobeに追加する必要があります。
